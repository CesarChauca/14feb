...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Cesar Chauca";
...
function showCountdown() {
  ...
  // Usando tus fechas base
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-12-20T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-12-20T00:00:00');

  function update() {
    const now = new Date();
    //calculo para "llevamos juntos"
    let diff = now - startDate;
    
    //constante de tiempo
    const msSeg = 1000;
    const msMin = msSeg * 60;
    const msHor = msMin * 60;
    const msDia = msHor * 24;
    const msMes = msDia * 30.4375; // Promedio de días en un mes

    let mesesJuntos = Math.floor(diff / msMes);
    let diasJuntos = Math.floor((diff % msMes) / msDia);
    let horasJuntos = Math.floor((diff % msDia) / msHor);
    let minsJuntos = Math.floor((diff % msHor) / msMin);
    let segsJuntos = Math.floor((diff % msMin) / msSeg);

    // --- CÁLCULO PARA EL ANIVERSARIO (Tu lógica actual mejorada) ---
    let eventDiff = eventDate - now;
    let eventDays = Math.max(0, Math.floor(eventDiff / msDia));
    let eventHours = Math.max(0, Math.floor((eventDiff % msDia) / msHor));
    let eventMinutes = Math.max(0, Math.floor((eventDiff % msHor) / msMin));
    let eventSeconds = Math.max(0, Math.floor((eventDiff % msMin) / msSeg));

    // --- RENDERIZADO ---
    container.innerHTML =
      `Llevamos juntos:<br>` +
      `<b>${mesesJuntos}</b>m <b>${diasJuntos}</b>d <b>${horasJuntos}</b>h <b>${minsJuntos}</b>m <b>${segsJuntos}</b>s<br>` +
      `<small>Nuestro aniversario:</small><br><b>${eventDays}d ${eventHours}h ${eventMinutes}m ${eventSeconds}s</b>`;
